...
- 283320f: remove `navbar` and `footer` props, pass `<Navbar>` and `<Footer>` as children of `<Layout>`
...
- 283320f: remove `feedback.useLink` and `editLink.component` option, remove `themeConfig` prop on `<Layout>` component
...
- 283320f: support `<Head>`'s `backgroundColor.dark/light` props colors in `rgb(...,...,...)` and `#RRGGBB` formats
...
- 283320f: content width could be configured via CSS `--nextra-content-width` variable
...
- 283320f: refactor `useMenuActions` to `setMenu`, `useFocusedRouteActions` to `setFocusedRoute`
...
- 283320f: - if `filePath` is falsy in client wrapper -> do not render `edit on github` link
...
- 283320f: allow `editLink`/`feedback.content`/`search`/`toc.backToTop` be `null` as value
...
- 283320f: add required `Layout.navbar`, `Layout.footer` props and optional `Layout.banner` prop, you don't need to pass them as `children` now
...
- 283320f: fix click on the arrow icon in the folder item in the Sidebar, was always consider clicked on `<a>` or `<button>` due
  `event.currentTarget`
